...
This project relies on a typical binary classification problem: there is only a label “credit_response” whose possible values are “good” and “bad” which refers to a bank customer who asked for a credit.
...
The *german.txt* file has been made as a slightly transformation of the  *german.doc* file.
The *key_map* is  extracted from the *german.txt*,  that is a kind of hash-map with two variables: the keys as "Axxx" format and the values as meaningful descriptions. Next, the glimpse of   *key_map* and   *credit_clear* dataframes.
...
credit_clear <- credit_clear %>% mutate(credit_response=ifelse(.$credit_response==1,"good", "bad")) 
head(key_map)
cat("\t\t\t\t\t\t\t\t\t\t*key_map head*\n\n\n\n\n\n\n\n\n\n\n\n\n")
glimpse(credit_clear)
cat("\t\t\t\t\t\t\t\t\t\t*credit_clear glimpse*\n")
...
The following **credit duration density plot**  shows that most of good credits last few weeks, whilst long time credits tend to be not so good.
...
The table above shows the summarized metrics: the accuracy is not   excellent. The sensitivity is good, the specificity is good too, so the FPR=`r {1-cm_glm$byClass["Specificity"]}` is quite low.
This approach seems to be quite satisfactory.
...
```{r model_RF, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}

# RF Algorithm Cross Validation to look for mtry best parameter
if(TRUE){ # SET "TRUE" IF  THE NEXT BLOCK IS SET FALSE
  metric <- "Accuracy"
  ...
  rf.train <- train(x,y, data=german_credit_train, method="rf",
                    ...
                    tuneLength=15, 
                    trControl=control)
  saveRDS(rf.train,"rfTrain.RDS" )
}
if(FALSE){ # SET "FALSE" IF THE ABOVE BLOCK IS SET "TRUE"
  rf.train <- readRDS("rfTrain.RDS") 
}
...
The model, after cross-validation, is trained by randomForest randomForest's library function to tune both ntree and randomly selected predictors. The metrics obtained with all predictors are quite similar to the GLM apporach. The accuracy (0.76) is not so satisfactory; the sensitivity better than GLM, but GLM wins on specificity .
In the second RF model, the nine most important predictors have been chosen for training: except a slightly reduction in sensitivity, there are improvements.
`r {knitr::kable(tab.rf_imp)}`  
...
As shown in the metrics table, when using all variables as predictors, there are 	worsenings compared to GML and RF except for the sensitivity
...
saveRDS(rpart.train,"rpartTrain.RDS")

}
if(FALSE){ # SET "FALSE" IF THE ABOVE BLOCK IS SET "TRUE"
  rpart.train <- readRDS("rpartTrain.RDS") 
}
...
The VarImp results are clear: the Rpart algorithm, for fitting, takes into account mainly six over nineteen variables; anyway the prediction after the retraining done with the five-variables subset of predictor i slightly better baut there is the same low  specificity, so this model hardly would be chosen. 

`r {knitr::kable(tab.rpart_imp)}`
...
This approach is based on the processing of the following formula:
  ...
  The NB approach using all predictors gives quite good results, but lose compared to GLM. The specificity is slighty better than RF
...
The variance distribution shows that beyond the first three component there is a great amount of variability, i.e. the principal three component have only the  `r {pr_comp}`% of total variability. On the basis of this evidence, no training/predictions have been done using PCA.
...
Prior to choose the best approach, we should keep always keep in mind the business requirements related to the particular project. In other words, the decision is to choose  the best model to maximize profits and minimize losses for the bank. These would be the main rules to follow:
  ...
  • To incorrectly predict a customer with good credit rating as bad, means to deny him the credit loan but there is neither any profit nor any loss involved in this case. High sensitivity is appreciated, but specificity shoud more important .
...
GLM seems to be a good approach: best  AUC and Accuracy, good Specificity, but has not the best  *sensitivity*; NB  has good *specificity* and AUC (68.5%) lower than GLM's AUC(70.3%) but higher that RF's AUC(66.5). In this case, a deep analisys should be done in order to understand the weight of *credit_amount* ( the most important variable in all methods) and look for a better tradeoff between sensitivity, specificity and AUC.
...
The data scientist should always keep in mind that the business requirement set is the main milestone when he works on whatever project. The techineques adopted in this project would aim to better explore the business rules and  the problem domain and eventually would find out the solution that best fits the use cases. Altough  GLM could be fair solutions for this use case, as seen in the results section, none of the models perfectly fits the problem: for example, further algorithms could be used and benchmarked (i.e. Neural networks); deeper analysis on important variable could be done and different seeds tested, ensemble tecniques should be helpful. Some or a combination of various approaches could drammaticaly improve metrics and reliability.  
