...
The *key_map* is a obtained from the *german.doc*, conveniently slightly handly modified, that is a kind of hash-map with two variables: the keys as "Axxx" format and the values as meaningful descriptions. Next, the glimpse of   *credit_clear* df.
...
The *credit_amount*, *credit_duration*, *age* variables are the numerical ones: the first  concerns the credit required, it shows the most variability among all variables, the second about the credit_duration in weeks, the third is about the wide range of appicant's age. The others are all categorical variables.

Next a simple plot showing the "good" and "bad" credit proportions: the first that should get the positive bank board evaluation has `r  {length(which(credit_clear$credit_response=="good"))/10}`% proportion an the second that should not the `r {length(which(credit_clear$credit_response=="bad"))/10}`%

```{r credit_response, echo=FALSE, message=FALSE, warning=FALSE}
...
The *credit duration density plot* shows that most of good credit last few weeks, whilst long time credits tend to be not so good.
...
**ML models** 
The following Machine learning algorithms have been chosen to fit train data and predict outcome on test data. These are among the most used, and fit quite well with the dataset; on the other hand the *knn* does not because it is not particularly effective on variables where there is a luck of hierarchical relationship between levels or numbers, so ithas not be chosen as model.  
...
```{r model_RF_tEST, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
